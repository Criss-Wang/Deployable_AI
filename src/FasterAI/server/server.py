...
from helper import (create_request, get_result,
                    get_cached_result, update_request)
...
- Make the consumer/worker dockerize so it can be easy to run multiple
  consumers using Amazon EKS, ECS, or other services

Things we have done:
- TTL
- persistence with RDB
- Caching request data to Redis
- Avoided long polling with WebSocket, creating separate namespaces for
  workers and client
"""
